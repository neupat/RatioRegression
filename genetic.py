...
from ratio_regression.program import _Program
from ratio_regression.fitness import _FITNESS_MAP, _Fitness
from ratio_regression.functions import _FUNCTION_MAP, _Function, SIG1 as sigmoid
from ratio_regression.utils import _partition_estimators
from ratio_regression.utils import check_random_state, NotFittedError
...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params, gen,
                     p_clean, p_vars, n_vars, p_const, n_const, number_of_gens):
    ...
    for i in range(n_programs):
        ...
        if parents is None:
            ...
            #Clean Up the Parent Program with a probability that depends on the Program length
            parent.cleanup(prob=p_clean(len(parent.program)))
            if method < method_probs[0]:
                # crossover
                donor, donor_index = _tournament()
                program, int_pars, removed, remains = parent.crossover(donor.program,
                                                                       random_state, donor.int_pars)
                ...
                # subtree_mutation
                program, int_pars, removed, _ = parent.subtree_mutation(random_state)
                genome = {'method': 'Subtree Mutation',
                          'parent_idx': parent_index,
                          'parent_nodes': removed}
            elif method < method_probs[2]:
                # hoist_mutation
                program, int_pars, removed = parent.hoist_mutation(random_state)
                genome = {'method': 'Hoist Mutation',
                          'parent_idx': parent_index,
                          'parent_nodes': removed}
            elif method < method_probs[3]:
                # point_mutation
                program, int_pars, mutated = parent.point_mutation(random_state)
                genome = {'method': 'Point Mutation',
                          'parent_idx': parent_index,
                          'parent_nodes': mutated}
            else:
                # reproduction
                program, int_pars = parent.reproduce()
        ...
        #Optimize n_const of the constant parameters with Probability p_const
        #Return Value is helpful for debugging
        if np.random.rand(1) < p_const:
            _ = program.optimize_n_consts(X, y, n=n_const)
        if np.random.rand(1) < p_vars:
            program.optimize_n_vars(X, y, n=n_vars)
        #_ = program.optimize_n_consts(X,y,n=1)
    ...
    #Print generation in shell if stdout is piped somewhere else.
    if sys.stdout != sys.__stdout__:
        pipe_loc = sys.stdout
        sys.stdout = sys.__stdout__
        print("Generation ", gen, " out of ", number_of_gens)
        sys.stdout = pipe_loc
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self._method_probs = None
        self._transformer = self.transformer
        self._programs = None
        self.run_details_ = None
        self._best_programs = None
        self._program = None

        #print all hyperparameters
        print('Symbolic Regressor Initialized with:')
        for attribute in ['n_const', 'p_const', 'n_vars', 'p_vars',
                          'population_size', 'generations', 'tournament_size',
                          'parsimony_coefficient', 'p_crossover', 'p_subtree_mutation',
                          'p_hoist_mutation', 'p_point_mutation',
                          'p_point_replace', 'init_depth']:
            print(attribute, ': ' + ' '*(40 - len(attribute)) + '\t'*9, getattr(self, attribute))
        print('p_clean', ': ' + 10*'\t', inspect.getsource(self.p_clean))
        print('\n'*3)
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for gen in range(prior_generations, self.generations):
            ...
            # print the six best Programs in every iteration. Very helpful to see what is going on
            if population is not None:
                print('_'*84)
                inds = (np.argpartition(fitness, 6)[:6])
                best_indices = np.argsort(np.array(fitness)[inds])
                for l in range(6):
                    print(str(l + 1) + "-best: \t \t ",
                          (np.array(population)[inds.astype(int)])[best_indices[l]])
                print('='*84)
                print('\n')
...
class SymbolicTransformer(BaseSymbolic, TransformerMixin):
    ...
    def fit_transform(self, X, y=None, **fitparams):
        ...
        """
        sample_weight = fitparams.get('sample_weight')
        if sample_weight is None:
